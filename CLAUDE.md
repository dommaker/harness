...
Presets (`src/presets/`) are pure data with severity-named keys (`errors`/`warnings`): `strict` and `standard` enable all constraints; `relaxed` enables only the 2 core constraints. Filtering lives in `mergeConstraints`; unknown preset names fall back to `standard` with a stderr warning.
...
- severity='error' violations MUST throw `ConstraintViolationError`, never silently pass

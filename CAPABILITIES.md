...
- skip 语义：约定未采用（capability_sync/docs_freshness/context_doc_sync/governance_presence 存在性探测）、context_files 约定已立但无目标（enabled-empty：required_dirs 缺失/空/含非字符串项，工单 84）、flag 未接线或检查输入不可得（git 取证失败，harness#182）→ skipped（satisfied=true，不阻断、不计入通率），trace 记 result:'skip'。context_doc_sync 与 docs_freshness 对三态判定同构。harness#182 起 skip 恒带原因（`CheckSkip{skip, reason}`）：原因落 `NormalizedOutcome.skipReason` → `ConstraintResult.skipReason` → CLI 跳过清单随行 / trace `skipReason`，「降级跳过（检查器失效/输入不可得）」与「约定未采用」在结果面可区分、可计数
- 输入契约（harness#182）：checker 经 `needs{evidence, contextFlags}` 声明输入，编排层与 checker-as-guard 在 evaluate 前经 `findMissingInputs` 比对 `CheckEnv.evidenceAvailable`（git-evidence adapter 透出 memo 内的 ok 位：stagedDiffAvailable/changedFileNamesAvailable）与 context 标志，缺输入统一降级为带原因的 skipped——「检查器失效 ≠ 对象合规」，不再对空 diff 假 pass、对 undefined flag 无声 skip；fail-open 保留但一律发声（catch 路径 = console.warn + pass+evidence 或带原因 skip，对齐 capability_sync 既有姿势）
...
- checker 行为口径（harness#184/#185）：`no_hardcoded_credentials` fail 恒带「文件:行号 — 规则名」证据行，diff 路径与内容扫描同口径豁免模板/文档类文件（`.env.example`、`.md` 样例等），与 `security secrets`（gitleaks 全历史，ADR-0031）分工 = commit 时增量正则扫描 vs 全历史兜底；`no_test_simplification` 限定测试文件（`.test.`/`.spec.`/`__tests__`）判定、删增配对豁免（净删除才判，重命名/改断言不拦）、新增 `.skip(`/`.only(`/`xit(`/`xdescribe(` 判违规。

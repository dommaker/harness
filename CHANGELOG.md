...
### Changes
- fix(cli): 数值旗帜脏输入全面 fail-loud——#152「数值旗帜装配口径」推广到剩余 7 面旗帜，统一解析器收口（#154，#152 resolution 第 3/4 点留账）——#152 只收了 `knowledge audit --threshold` 一处，其余数值旗帜的脏输入后果分两类留在账上：**静默降级**（`constraints report` 三阈值 NaN 过 `!== undefined` 进阈值槽 → 比较恒 false → 候选静默减少、退出码 0；`knowledge --limit` NaN 被 `|| 20` 兜成缺省量；`failure --limit` NaN falsy → 静默不截断；门禁 `--bundle-threshold` NaN falsy → 落 gate 缺省）与 **fail-closed 但文案自证异常**（`passes-gate --coverage-threshold` 打出「覆盖率不足 (x% < NaN%)」、`review --min-reviewers` 恒不通过）。落地：唯一解析器 `src/utils/numeric-flag.ts`（`parseNumericFlag` 判别 + `requireNumericFlag` 抛出 + `NumericFlagError`，trim 后全数字/一段小数才收，`parseInt`/`parseFloat` 的前缀截断 `'30abc'→30`、`'1e2'→1` 同脏值拒），CONTEXT.md 口径条目指名为正本、不建第二套（#152 的 `assembleShortContentThreshold` 改为委派同一规则，函数面与报错文案保留）。两消费形：定义表 `mapActionArgs` 站点（constraints report 三阈值、performance 两阈值、review `--min-reviewers`）走 `requireNumericFlag` 抛出、`bin/harness.js` 在 `runDefinition` 捕获映射 `usage-error`（定义表是纯数据模块返回不了 CommandResult，装配失败通道只此一条，bin 不新增单命令知识）；命令模块站点（`knowledge search --limit`、`failure list --limit`、`passes-gate --coverage-threshold`）走 `parseNumericFlag` 就地 `logError` + `usage-error`，全部先于 store/recorder 构造与任何执行（脏值零落盘）。**fail-closed 两处不保留、一并收敛 fail-loud**（票面授权的裁定）：脏输入属用法错误、自证异常的文案即缺陷。顺带修一处同源暴露的 io 漏传：`knowledgeSearchCommand` 调 `knowledgeSearch` 未透传注入 `io`（生产路径经缺省 `processIO` 无差，测试注入面静默漏字）。**对外行为变化**（定级**非 breaking / patch**，判据同 `[1.6.0]` 删 `--max-retries` 与 #152/#163 的「静默失效 → 报错」先例）：合法取值路径逐字不变（未传 vs 显式缺省值输出逐字相等、显式 `0` 不被当未传兜掉——`knowledge --limit 0` 由兜回 20 改为真按 0 判定、其余站点 0 语义本就成立），命令语法、旗帜集合、`--json` 字段集合、公共导出面零变化；变化的只有脏输入面——四组静默降级站点由「退 0 + 结果少几条」变为报错退 1，两处 fail-closed 站点由自证异常文案变为可定位报错（review 1→1，passes-gate coverage 脏值 0→1）。边界不动：门禁/引擎构造器侧的 falsy/`??` 约定是库面向语义（显式 `0` 在 performance 门禁内仍 = 不检查该项），与 #163 同例不重复兜；`status --hours` 是死旗帜另族另票。测试：新增 `src/utils/__tests__/numeric-flag.test.ts` 26 项（解析规则矩阵）+ `src/cli/commands/__tests__/numeric-flag-assembly.test.ts` 30 项（定义表逐旗帜装配、命令入口 fail-loud、端到端真实 spawn `bin/harness.js`：脏输入 → 退 1 + stderr 点名旗帜 + stdout 零字节 + 零副作用落盘，未传 vs 显式缺省逐字对拍，显式 `0` 语义逐站钉住），**先红后绿**（红基线：模块缺失套件不可运行 + 脏输入退 0）；`knowledge-view.test.ts` 闸 1 豁免表 2→3 行（search --limit 入参闸门，理由随表注）；`public-value-surface`/`public-exports`/`public-type-surface` 三闸零清单改动。验收：`npx jest --runInBand` 全量 186 套件 2638 passed、`tsc --noEmit` 与 `eslint src` 零错、`node bin/harness.js check` 通过
- feat(knowledge): `KnowledgeAudit` 构造器对三个数值槽的脏输入 fail-loud——显式传入 NaN / ±Infinity / 负数时构造期抛 `TypeError`（#163，#155 留账、#152 同构「转不出来就响」；维护者已裁走法 1 构造器 fail-loud，收窄类型与引擎兜底回缺省两走法在 #155 明确否决、本票不复活）——三槽（`shortContentThreshold`/`staleDays`/`promotionBlockDays`）经 `resolveThresholds` 的 `??` 兜缺省，而 `??` 只兜 `null`/`undefined`、NaN 照单全收：NaN 进 ctx 后三处比较恒 false，`short-content`/`stale-entry`/`promotion-blocked` 三条规则同时静默关闭，构造方拿到零报错的正常审计结果。落地：守卫加在 `resolveThresholds` 上游的构造期（新增模块内辅助 `assertValidThresholds`，两处不重复兜），任一槽**显式传入**的值非有限或为负即同步抛 `TypeError`、message 含槽名与实参；未传/显式 `undefined` 走缺省逐字不变，显式 `0` 合法且按 0 生效（不被兜回缺省，口径同 #152 CLI 面「显式 0 不被当未传」），正有限值（含非整数）行为不变；`AuditOptions` 类型形状不动（仍 `number` 可选，走法 2 已否决）。CLI `knowledge audit --threshold` 行为零变化——上游装配点已挡脏输入，守卫不会被 CLI 路径触发。**对外行为变化 1 处**（定级**非 breaking**，判据同 `[1.6.0]` 删 `--max-retries` 与 #152 的「静默失效 → 报错」先例，落地复核成立：仓内 ingest 不传 options、CLI 已被 #152 挡住、studio 不传数值槽——零已知消费者受影响）：库面直调 `new KnowledgeAudit(store, {…})` 传脏值由「静默关掉三条判定」变为构造期报错；合法取值的行为字节不变。测试：`src/knowledge/__tests__/audit.test.ts` 新增「构造器数值槽守卫（harness#163）」describe 17 项——三槽 × NaN/Infinity/-Infinity/-1 各抛 `TypeError` 且 message 含槽名与实参（**先红后绿**：实现前 12 条全红「did not throw」）、不传与显式 `undefined` 同果走缺省、显式 `0` 时 short-content 不触发且 `staleDays: 0` 下昨天引用的条目即判 stale（两槽各证「0 不被兜成缺省」）、正有限非整数（30.5）边界随入参移动。CLI 面闸 `knowledge-audit-threshold.test.ts` 与 `bin-exit-mapping.test.ts` 零改动全绿。文档：`src/cli/commands/CONTEXT.md` 数值旗帜装配口径段末「库面向的守卫另票」更新为本票结论、`src/knowledge/CONTEXT.md` 构造入参口径同步。验收：`npx jest` 相关 22 套件 554 项全绿、`tsc --noEmit` 与 `eslint src` 零错
- perf(sync-docs): 按需读取——capability-listing 模式对源码树**零内容读**，描述提取改并发而清单顺序逐字不变（#147，#140 B 票；ADR-0023 未收口三处之二）——`sync-docs` 入口此前无条件对所有源码目录全树扫描：逐文件 `await readFile` **只为取首行注释作描述**，而这份描述在 capability-listing 格式（本仓即是）下根本不进任何输出——该格式的判定面是计数，条目级比对与表格写回都住 file-table 分支，`currentModules` 在 listing 分支是自扫自用的死数据；串行 await 又在大仓上线性放大。改造形状 = **先判定「本次要什么」再决定「读什么」**：① `index.ts` 把 CAPABILITIES.md 的格式判定移到扫描之前（只搬读取路径，判定/漂移检测/写回内容一字未动），listing 分支自此整棵源码树一份 `.ts` 内容都不读；② 要描述的 file-table 分支照旧取描述，但 `project-reader.ts:scanSourceModules` 拆成两步——先按目录遍历序定清单（`name`/`file`），再经新增的 `extractFileDescriptions()` **并发**取数并**按原索引回填**，上限固定 16（按票下裁决不做成配置面）。**输出顺序逐字保持现状**是票面裁决：该顺序对外经 `--check` 的行级比对与人看 diff 可见，改字典序要付一次全仓漂移的代价（已裁不值），故「按并发完成顺序 append」按行为漂移对待而非优化。**对外行为变化 0 处**：判定、人读与 `--json` 两面、stdout 与退出码逐字节不变，`ModuleInfo` 与包公共面零改动（`extractFileDescriptions` 是本模块公共面，不进 `src/index.ts`）。实测数字对照（同一插桩、同一棵树）——本仓 listing 模式 `.ts` 内容读 **290 → 144**、命中文件 **161 → 144**，被消掉的 **146** 份正是扫描分支的全部读取（= 源码树内 `.ts` 排 barrel/`.d.ts`/`__tests__` 的文件数，与生成的模块表 146 行同源），余下 144 份是 CONTEXT 导出面采集（#142，票面范围外、一字未动）；同树改判为 file-table 后两版构建读取数同为 **290**（本票不削减该分支的读取量，只把它并行）、在途并发 **1 → 16**、`--check` 三轮 **430/453/420ms → 408/398/407ms**（收益上限是 libuv 线程池缺省 4，票面承诺是「不再串行放大」而非倍增）。等值验收用真构建两版对撞：改动前（HEAD）与改动后两版 `dist` 跑同一棵树 `sync-docs --check`，**stdout 与退出码逐字节一致**；写回面同法——删掉 CAPABILITIES.md 让两版各自生成 146 行模块表，两份产物**逐字节一致**（行序与每条描述都对得上）。**测试**（新增 3 套件 / +12 项，全量 184 套件 2560 passed，改前同树同命令 181 套件 2548）：`__tests__/sync-docs-read-count.test.ts` 形状照 `check-read-count.test.ts`（`fs.readFile` 与 `fs.readFileSync` 记件、原地清空防假绿）——listing 模式 `.ts` 内容读 = 0（**改前实测红：4 份**）、file-table 反证每份源文件恰读一次且描述确实进对外 JSON、CAPABILITIES.md 缺席走同一分支、listing 整张读取表冻结（表上 `CAPABILITIES.md=5` 是**本票范围外的既有重复读**：入口一次异步读 + 计数核对按每条计数规则各读一次，如实登记、不为它改动）；`sync-docs/__tests__/module-descriptions.test.ts` 用替身把「起始序」与「完成序」刻意错开（起始靠后的读更快落地），钉住并发真发生、在途 ≤16、清单顺序 = 读取发起顺序、每条描述与自己那份文件配对（串位即红）、读失败回退文件名的兜底语义不变；`__tests__/sync-docs-module-order.test.ts` 冻结对外可见面（写出的表格行序 = 目录遍历序 = 扫描清单顺序，且两次运行逐字复现；**不写字面顺序**——`readdir` 返回序由文件系统决定，本仓 tmpfs 上是字典序、ext4 上是哈希序，写死等于把断言建立在机器上，故以遍历序为口径正本）。**反证已做**（四态各自实测）：拆掉 listing 的扫描闸 → 计数闸 2 项转红、file-table 两项照绿（闸不空转）；改按完成顺序 append → 配对/顺序 3 项转红；去掉并发上限 → 上限那条转红；退回串行 → 并发与顺序 2 项转红（与改前状态同形）；四态全部还原后 12 项复绿。验收：`npx jest` 184 套件全绿（2560 passed / 6 skipped）、`npx jest --coverage --no-cache` 两态对撞四档全升（全局 lines **96.18% → 96.24%**、stmts 95.35% → 95.4%、branches 84.23% → 84.32%、functions 82.06% → 82.12%；`project-reader.ts` stmts/lines **79/81**、branches 10/17、functions 14/14，`sync-docs/index.ts` stmts 198/210）、`tsc --noEmit` 与 `eslint src` 零错、`npm run build` 绿、`node bin/harness.js check` 通过、`sync-docs --check` 零漂移。文档：`src/cli/commands/CONTEXT.md`「约定」新增「先判定本次要什么再决定读什么，且清单顺序是对外面」条。范围外按票面未碰：输出顺序改字典序、`agents-syncer.ts`（自有 sort）、`spec-baseline-check`（#146 已落）与 `check.ts getSmartHint`（#148）的读取收口、并发上限配置化
- perf(spec-baseline-check): 一次运行只扫一遍全仓——消除「前置条件数 × 关键词数 × 源文件数」的逐条重扫（#146，#140 A 票；ADR-0023 未收口三处之一）——工单 19 只把 `checkCodePattern` 的**文件遍历**收敛成一次 `walkFiles`，读取仍留在逐关键词的循环里：每个关键词把全部源文件 `readFileSync` 一遍（一条前置 K 个关键词就重读 K 遍），`checkDependency` 又逐条前置重新读取并 `JSON.parse` 一次 `package.json`。三条验证路径（文件存在性 / 依赖 / 代码模式）自此一律从**命令级共享索引** `createBaselineIndex(projectPath)` 取数：全仓 `.ts`/`.js` 内容扫描一遍建成 `路径 → 内容` 表（读失败的文件不入表 = 原逐文件 try/catch 跳过同义）、`package.json` 读解析一次且**失败态同样入库**（不逐条重试）、同一落点的存在性探测只 stat 一次；逐条前置与每个关键词只在索引上重放，匹配语义（`content.includes(kw)` 逐文件计数）逐字等价。索引**懒建**（没有前置要某类数据时该类读取一次都不发生，故纯存在性/纯依赖型 spec 不触发全仓扫描）且**运行结束即弃**——按 #140 triage 裁决 5 **不扩 `RunEnv` 公共面**（本命令是独立命令、不是 `harness check` 的子步骤，「同一口径取一次」命令内 memo 自足），四个验证器函数的 `index` 形参全为模块私有面。**对外行为变化 0 处**：判定、证据文案、stdout（人读表与 `--json` 两面）与退出码逐字节不变，`RunEnv`/`CheckEnv`/包根导出清单零改动。实测数字对照（同一插桩跑同一夹具：7 条前置含 2 条依赖 + 3 条代码模式各 2 关键词，6 份源文件）——内容读总数 **39 → 8**、`package.json` 解析 **2 → 1**、每个源文件 **6 → 1** 遍、全仓目录列举 **9 → 3** 次、存在性探测 **3 → 2** 次；关键词扩到 15 个（7 条前置）时改前每文件 15 遍、改后仍是 1 遍（计数闸第三条：两次不同条数的运行剔掉 spec 自身后读取表逐条相同）。**测试**（新增 1 套件 / +5 项，全量 181 套件 2548 passed）：`__tests__/spec-baseline-read-count.test.ts` 形状照 `check-read-count.test.ts`（真执行、只在 `readFileSync` / 只读 `openSync` / `existsSync` / `readdirSync` 上记一笔）——整张读取表与目录扫描表逐条冻结（表上无 `dist/`、`node_modules/`、`.harness/` 条目即跳过口径未变的证据）、「除冻结条目外任何项目文件至多读一次」、N×K 缩放不变式、懒建（纯依赖 spec → 零目录列举）、`package.json` 读取失败态入库（两条依赖前置只尝试读一次且各报一次「无法读取 package.json」），每条断言带正对照（三条验证路径确实都真评估，防闸空跑）。**反证已做**：三枚 memo（源码索引 / package.json / 存在性）逐枚拆开后——拆源码索引 3 项转红、拆 package.json 4 项转红、拆存在性 1 项转红（冻结表用例），三枚全在时 5 项全绿，还原后复绿；改前状态另以 `git show HEAD` 的源文件跑同一枚计数闸量得（上表 39/2/6/9/3 即该轮输出）。等值验收用真 CLI 两版构建对撞：改动前构建（HEAD）与改动后构建分别跑同一夹具的普通模式与 `--json` 模式，**stdout 与退出码逐字节一致**（含 `✗` 行与黄色证据行，退出码同为 1）。验收：`npx jest` 全量 181 套件全绿（2548 passed / 6 skipped，改前同树同命令 2543 passed）、`npx jest --coverage --coverageReporters=json-summary` 两态对撞：全局 lines **95.37% → 96.18%**、stmts 94.58% → 95.35%、branches 83.39% → 84.23%、functions 81.81% → 82.06%，`spec-baseline-check.ts` 自身 lines **91/159 → 168/176**、branches 43/82 → 74/86、stmts 95/165 → 173/182（余下未覆盖的 9 条语句全是本票未动的既有分支：三处「无法从描述中提取 X」与两处「部分存在/依赖缺失」证据行、`implMatch` 追加关键词一支、`formatTable` 的空表早退与两条 `未满足` 排版行。**两态数字一律 `--no-cache` 量得**——jest 的 transform 缓存会跨源文件切换端出旧的插桩表，带缓存复测时同一棵树上读到过 159/165 与 176/182 两组互相矛盾的本文件分母，据此写下的任何对照数字都不可信）、`tsc --noEmit` 与 `eslint src` 零错、`npm run build` 绿、`node bin/harness.js check` 通过。范围外按票面未碰：判定逻辑与关键词/包名提取正则、证据文案（零改动）、`sync-docs` 与 `check.ts getSmartHint` 的读取收口（#147 / #148）、`traces.log` 相关读取。文档：`src/cli/commands/CONTEXT.md`「约定」新增「一次命令运行内同一份数据至多取一次」条
- feat(init): 新增第 9 个脚手架站点 `.git/hooks/pre-push`——推送前的本地全量防线（#144，#132 grilling 曝光的已知缺口）——pre-commit 可被 `git commit --no-verify` 绕过，而绕过后的内容此前**无任何本地拦截**直接离机（全仓无 pre-push hook）。落地形状走 #132 的 scaffold 三态模型：`scaffold.ts` 新增 `prePushHookFile` 工厂（目标路径 + 三态文案 + `mode 0o755`），正文住 `scaffold-templates.ts`（`PRE_PUSH_SNIPPET` / `renderPrePushHook`，与 pre-commit 同一形状：落盘正文 = shebang 头 + 打印片段，#103 判据），`init` 的 `setupGitHooks` 退化为「一条 plan 装两道 hook」，跳过策略仍在命令层（`--no-git-hooks` 一并覆盖本站点，**不新增任何跳过旗帜**）。三道决定按 grilling 定论钉死并写进 hook 正文：**① 不做增量**——不解析 pre-push 从 stdin 收到的 `<local_ref> <local_oid> <remote_ref> <remote_oid>` 清单，无论推什么（含删远端分支这种 stdin 空清单）整仓过一遍，钩子内两条命令一律 `< /dev/null` 不让子进程吃掉 ref 清单（`npx` 缺包时的交互确认一旦读到 ref 行会把 push 顶成一条看不出所以然的失败）；**② 不内建逃生机制**——无环境变量开关、无超时、无「慢则降级」，唯一口子是 git 原生 `git push --no-verify`；**③ 定位不变**——本地 hook 是自检与提醒，门禁真正的落点仍是服务端 CI（`--ci` 维度，见 #143），pre-push 不替代它。**对外行为变化 3 处**（定级 **minor**，加性站点 + 两处文案）：① 带 `.git` 的项目跑 `init` 现在多落一个可执行的 `.git/hooks/pre-push`，此后每次 `git push` 前先跑全量 `harness check`（不带 `--staged`）+ `harness validate`，任一失败非零退出挡住推送——**含新工程**：`init` 生成的缺省 `checkpoints.yml` 两条检查点跑 `npm run build` / `npm test`，脚本缺失的新项目真机实测 `git push` 会被 `❌ Validate failed` 挡下，这与同一份 `init` 生成的 `harness-check.yml` / `.gitlab-ci.yml` 里 `validate` 步骤的服务端语义完全一致（本地只是早一步撞到），出路是补脚本、改检查点或 `--no-verify`；② `init` 收尾「下一步」第 3 行由「正常开发，每次 git commit 会自动检查约束」改为写明两道分工（commit 查暂存的、push 查整仓的，**重复是设计使然**）；③ `init --print-snippets` 新增 `Git pre-push hook:` 段（引导语同叙事），既有 pre-commit / CI 段逐字不变。pre-commit 既有内容按票面范围外**一字未改**，仓库自身的 `bin/install-precommit-hook.sh`（`npm run hooks:install`）仍是单道。**测试**（+1 套件 / +14 项，全量 179 套件 2541 passed）：`scaffold.test.ts` 站点表 8→9（目标路径逐字冻结、`0o755` chmod、落盘态/在场态文案逐字、同源闸与「改落盘正文不改片段」反证探针扩到两道 hook 各一条），另加两枚静态闸钉住决定不被悄悄改回——「恰好 check + validate 两道」（`check` 不得带 `--staged`、出现第三条即红）与「不解析 stdin ref、无环境变量开关、无超时」；`init.test.ts` +3（可执行位、落盘正文形状、已存在时不覆盖且打印同源片段）；`init-ondisk.test.ts` 整屏基线加一行、`.git/hooks/pre-push` 落盘**字节级冻结**（同 `GITLAB_PLAIN` 手法：改模板必须同步改基线，这个「同步」动作正是拦住门禁被削弱的地方）、第二趟第三态与「打印即磁盘正文」扩到 9 站点、第三趟用户接管表含本站点且一字节不动；新增 `pre-push-hook.test.ts` 走**真 git 真 push**（临时仓库 + 本地 bare 远端 + PATH 上的记账替身 `npx`，装上的是 `renderPrePushHook()` 正本而非手抄）：两道全过→push 成功且调用序列恰为 `check`→`validate`、`check` 失败→非零退出且输出点名 `❌ Iron law check failed` 且远端一格未收、`validate` 失败→同样挡住（不是只看 check）、`--no-verify`→push 照过且钩子整个人没被调用、删远端分支（stdin 无 ref 清单）→仍整仓跑一遍。真机另跑一遍全链（`node bin/harness.js init` → 装上 hook → 真 `git push` 被挡 / `--no-verify` 放行 / 冲突分支与 `--print-snippets` 打印即落盘正文）。**同源反证已实测做正**：只往 `renderPrePushHook()` 追加一行 `passes-gate` 而不动打印片段 → 5 项转红（含 #103 同源闸与字节冻结基线），还原后全绿。**票面范围外未碰**（另开票）：GitLab CI 接线缺口、服务端门禁改动、增量/按 refs 范围检查。验收：`npm test` 179 套件全绿、`tsc --noEmit` 与 `eslint src` 零错、`node bin/harness.js check` 通过（铁律 3/3、指导原则 3/3）、`sync-docs --check` 本票文档已同步。文档：`src/cli/commands/CONTEXT.md` 站点口径 8→9（「pre-push 为已知缺口，另开票」改为已落地）并新增「本地防线两道」条
- feat(docs): CONTEXT.md 新鲜度判定由 **mtime 比较**换为「核心导出」节与目录导出面的**内容对撞**，并进 governance CI（#142，架构评审 2026-09-14 批次；判定形状对位 ADR-0009，决策记 ADR-0025）——旧判定是「目录内最新 `.ts` 的 mtime 比 `CONTEXT.md` 新即报过时」，三种失真全部实测过：改一行注释算漂移；**什么都不改、只碰时间戳也算**（`git checkout`/`stash`/cherry-pick 都会刷新源文件 mtime——票面证据：`src/cli/commands/CONTEXT.md` 当天被 6 张票逐个更新、`git status` 干净、内容与实现相符却被报）；接口真变了反而漏报（文档晚写一次就整目录洗白）。本会话同一命令在同一棵树上两次跑出**不同集合**（票面 4 处 vs 实测 6 处且与票面不重合），而 CI runner 是全新 clone、mtime 由 clone 顺序决定（同目录 `CONTEXT.md` 排序恒先于小写 `.ts`）→ 恒假阳性。另一半病灶是无人报警：`harness-governance.yml` 只 `sync-docs`（写入自愈）后对 `CAPABILITIES.md` 的 diff 报错，CONTEXT.md 无论漂多远 CI 不红——票面那 4 处就是这么攒出来的，而 agent 读代码前先读 CONTEXT.md，读到不实陈述直接下游误判。**新增纯判定** `src/core/constraints/context-reconcile.ts`（包内模块，不进 `src/index.ts` 导出面）：`reconcileContext({contextMdContent, exportSurface, barrelExports?})` → `{declaredSymbols, ghosts, unlistedBarrelExports, hasCoreExportsSection}`，与 `capabilities-reconcile` 同一套覆盖/幽灵语义、同一个「判定不吃 fs、清单由调用方组装」的形状，不另造第二套对照逻辑；两个口径正本同为纯函数——`parseDeclaredExportSymbols`（只认 bullet **行首**反引号标识符，`/` 连接视为多声明，形参列表与 `（type）`注解剥掉，剥完非合法标识符的整条跳过 = `types.ts`/`audit-scoring`/`constraints/` 这类模块名条目）与 `parseExportStatements`（声明形 / `export { a, b as c }` 交导出名与本名 / 逐项与块级 `type` / `export default` / `export *` 的待解析 spec）。**fs 采集在 cli 侧**（`sync-docs/context-syncer.ts` 新增 `collectContextExportSurface(dirPath, projectPath)`：`findTsSourceFiles` 排 `__tests__/`、`.d.ts`，`export *` 与具名再导出沿仓内相对 spec 解析，越出 projectPath/命中 node_modules/跳过目录不并入，按正本声明剔除类型-only 并**核对具名再导出的本名是否真在正本里**——反证撞出这条：虚再导出会洗白幽灵，改名恰恰是最该抓的情形；环上核不到就从宽不判）。**口径不对称**（票面验收 4 要的可操作口径）：幽灵方向值与类型一视同仁（文档写 `HookConfig`（type）就必须真有其物，且「某内部文件仍导出它」不足以救——导出面是全目录并集，改名要改到声明处），覆盖方向只要求 barrel 的**值**符号逐个可见（按字面要求全部再导出符号登记需往 12 份文档补约 500 个类型名，`core` 单个 barrel 就 63 项，而类型面早有 `public-type-surface`/`public-value-surface` 两道闸冻结清单），无 `index.ts` 的目录跳过覆盖方向（内部 seam 登记与否是文档裁量，`release/CONTEXT.md` 是先例）；「出现在该节」按标识符边界匹配、inline 提及即算登记，但**其它 `## ` 节的正文不救**未登记。**级别内建、不加独立开关**（票下 Brief 裁决 3）：内容漂移 → `--check` 判 fail 且 `reason` 直接点名文件与符号；mtime → 只打提示、不影响退出码，`process.env.CI` 置位时连提示都不给；写入模式**不改写 CONTEXT.md**（散文不可机械生成）、退出码面不变；`--json` 新增 `contentDrift: [{dir,file,ghosts,unlisted}]` 与 `summary.contextContentDrift`，`contextStale` 保留但自此只装 mtime 提示（CI 下恒空）。**进 CI**：`harness-governance.yml` 在 CAPABILITIES diff 门之后加 `Gate on stale CONTEXT.md`（`node bin/harness.js sync-docs --check`），按 #120 方案 A 先例清完存量**直接变红**、不给 warning + 限期档（给了就等于没接——第一批误报会把它磨成「预期失败」）；不套「写入 + diff 门」那套（那是可机械自愈的表格格式专属，CONTEXT 是散文）。**存量清理按闸的全部报出执行**（票面写 4 处，接 CI 前必须清干净）：7 个目录 32 项（31 未登记 + 1 幽灵）——core 12 / failure 7 / gates 5 / knowledge 3 / monitoring 3 / context 1 是公开值符号真漏登记，逐条核过消费者与签名后补进各自文档（例：`getTraceCollector`/`configureTraceCollector`/`createAnalyzer` 此前只在「约定」里被提到，不进「核心导出」就永远不可见）；`cli/commands` 那 1 项是 bullet 行首反引号写的是 CLI 子命令名 `command`（非导出符号），按「行首 = 声明」约定加散文前缀，句子一字未删。**逐条核对**（闸只判符号面，散文靠人读；hooks/knowledge/monitoring/release/cli/commands 五份全文对撞实现）另出**五处不实陈述已改文档**（hooks 的类型正本在 `src/hooks/types.ts` 而 `src/types/` 无 hook 类型、两条已失效的消费边（core / CLI 初始化）、`HookRegistry` 方法枚举漏 3 个；monitoring 读入口漏 `getStats()`、`ContextTracker` 的 `context/session-manager` 消费边未列；knowledge 的「依赖 `src/types/`」不实、`audit-scoring`「不进导出面」对类型面为假；release 的同步闸门归属写错）**+ 五处「文档正确、代码待改」不改文档、另开缺陷票**（#159 hooks `HookConfig.enabled`/`errorStrategy` 零读取点、#160 release 清单同步闸门只做正向 `toContain`、#161 knowledge 三条：`absent` 报进 errors 与 `utils/frontmatter.ts` 正本裁决相反 / 外部内容三层防御第二层 `formatForPrompt` 生产零接线 / `rule` 降级边界差一）；十处判定与证据逐条登记在 #142 票面，各文档原句就地标「现状偏差」并指回缺陷票。**对外行为变化 2 处同向**（定级 **minor**，标 `!`）：① CONTEXT 内容漂移自 `--check` 起判非零退出并点名文件与符号（此前永不报警）；② 「源码比文档新」不再判失败（此前会），CI 环境下不再产出该提示——判定面是从时间戳换成内容，不是单纯放松或单纯收紧。**消费方影响单列**（release note 必提）：升级后 `sync-docs --check` 对所有消费方变严，studio 侧 49 个 CONTEXT.md 将获得这套新判定（其修复不在本票）；已核实的缓冲是 init 生成的消费者工作流里该步骤带 `continue-on-error: true`（GitHub）/ `allow_failure: true`（GitLab），故消费方 CI 不会因本变更直接红。**测试**：新增 3 文件 36 项（19 + 5 + 12）——`core/constraints/__tests__/context-reconcile.test.ts` 19 项（判定与两口径，夹具取仓内真实写法，零 fs）、`sync-docs/__tests__/context-export-surface.test.ts` 5 项（采集、类型分型、`export *` 展开、跨目录正本核对、虚再导出不计入）、`__tests__/sync-docs-context-drift.test.ts` 12 项（fail 点名、mtime 降级与 CI 抑制、`--json` 形状、写入模式不改写）；既有 `sync-docs-agents.test.ts` 一处断言随措辞改字（`可能过时` → `源码比文档新`），其判失败位点从 mtime 换成缺失判定。**票面验收 2 的反证两种都跑**：① 只改声明侧类名（`ContextTracker` → `ContextSink`）不动 barrel 与文档 → `CI=true sync-docs --check` rc=1 并指名 `src/monitoring/CONTEXT.md` 与符号；② 只往 `src/release/index.ts` 加一行公开值再导出 → rc=1 并指名 `src/release/CONTEXT.md` 与 `HARNESS_PACKAGE_NAME`；两者还原后 rc=0 且输出「所有文档都是最新的」。验收：`npm test` 178 套件 / 2527 passed / 6 skipped 全绿、`npx jest --coverage` 全局 **94.58% stmts**（#141 的 94.56% 不降；branches 83.36%、functions 81.79%、lines 95.37%；`context-reconcile.ts` stmts/lines/functions 100%、branches 94.11%）、`tsc --noEmit` 与 `eslint src` 零错、`npm run build` 绿、`node bin/harness.js check` 通过（铁律 3/3、指导原则 3/3）、`sync-docs --check` 在 CI 与非 CI 两态下均干净。文档：新增 ADR-0025（含**绕闸限度**登记：删整节或删 bullet 可消漂、无该节的文档两方向都不判、导出面是正则解析源文本故模板字符串里行首 `export ` 会造成假阴）；`src/core/CONTEXT.md` 加新模块条目与 barrel 公开面条目、`src/cli/commands/CONTEXT.md` 记采集侧归属本层与降级语义，hooks/knowledge/monitoring/release/core/failure/gates/context 八份随核随改。范围外按票面未碰：`--agents` 半张闸（实测 AGENTS.md 零漂移）、`capabilities-reconcile` 既有判定、非 barrel 内部导出的登记要求、studio 侧修复。
- chore(cleanup): `effective-set` 的 `'throw'` 模式删除——共享筛选器的第二个模式失去生产调用方（#141，架构评审 2026-09-14 候选 1 步骤 3 的直接产物；判据 ADR-0022）——`filterEnabledEntries(knownIds, entries, {onUnknownId})` 是「config.yml 引用了哪些 id、哪些被 `enabled:false` 裁剪」的唯一实现（ADR-0001 收口），两个模式按语义分而非按调用方分：`'collect'` 归约束侧 `mergeConstraints`、`'throw'` 归门禁侧 `getEffectiveGates`。后者连同 config.yml 的 `gates:` 声明式面已随候选 1 收缩删除（commit `209511e`，理由记在 ADR-0002 文末「后续变更」），于是 `'throw'` 分支与 `unknownIdError` 选项**只剩自己的单元测试在供养**——一处「为两个消费方共用而抽出的共享件」退化成单消费方 + 一个无人使用的分支，正是 ADR-0022 定义的那类漏收面，只是这次由候选 1 的删除动作**新造**出来。**删**：`FilterEnabledEntriesOptions` 整个接口（唯一必填成员是 `onUnknownId`，单成员联合是空转开关）、`unknownIdError` 选项、函数体内的 throw 分支；签名退化为 `(knownIds, entries)` 的单一语义，`unknownIds` 保留（`mergeConstraints` 在用它做诊断）。**对外行为变化 0 处、零公共面变化**：`effective-set` 是包内模块，不在五个 `exports` 入口的名字清单里，三道公共面闸（`public-exports`·`public-type-surface`·`public-value-surface`）清单零改动且全绿。门禁侧将来若恢复声明式配置面，重建成本 = 一个 `if` + 一条错误消息，整段实现与测试在 git 历史里。**测试**：throw 那一组 3 条用例删除，collect 侧 4 条断言逐字保留（调用点随签名去掉第三参数）；按 ADR-0022 决策 2 的先例（AC-007 改写为钉新形状）补三枚「新形状钉」——`filterEnabledEntries.length === 2` 运行期钉 + 两枚 `@ts-expect-error` 编译期钉（第三参数不再是合法入参、`FilterEnabledEntriesOptions` 不再是可导入类型），**先红后绿**：实现前实测 `TS2554 Expected 3 arguments, but got 2` 四处 + `TS2578 Unused '@ts-expect-error' directive` 两处令套件红。**票面验收 4 的反证两种都跑**：① 往函数里加一条真·无主分支（`process.env` 门控、签名不变）→ 7 项全绿，说明清掉的是无主分支而非把断言挪了地方；② 把删掉的选项按可选形参复原 → 两枚编译期钉各顶一次 TS2578、套件红，说明形状冻结有牙（挡得住「悄悄改回来」，ADR-0003 本意）。**同型面登记（票面第 4 条：只登记不顺手改）**：`src/utils/jsonl.ts` 的 `JsonlBadLinePolicy = 'skip' | 'throw'` 与本票病灶同形——11 个生产调用点一律传 `'skip'`、`'throw'` 只被自己的单元测试供养，但它**不属漏收**：工单 #82 裁决 1 明文保留该选项（「既不让静默丢数据成为框架缺省，也不强制统一策略」），且 `jsonl-skip-disposition.test.ts` 有一条活闸钉住「`'throw'` 调用点目前为零、出现即逐个裁决」；本票病灶两者皆无（无裁决记录、无 tripwire），故一删一留。另登记 `src/knowledge/lifecycle.ts` 的 `recordReference(…, source?: 'human' | 'auto')`——本仓唯一生产调用点只传 1 个实参，但 `KnowledgeLifecycle` 经包根导出、studio 侧消费未核实，删不删须先双仓查。验收：`npm test` 175 套件 / 2491 passed / 6 skipped 全绿、`npm test -- --coverage` 全局 **94.56% stmts 与 #139 基线逐位相同不降**（`effective-set.ts` 四项 100%）、`tsc --noEmit` 与 `eslint src` 零错、`npm run build` 绿、`node bin/harness.js check` 通过（铁律 3/3、指导原则 3/3）。文档：`src/core/effective-set.ts` 头部与 `src/core/CONTEXT.md` 的「门禁侧曾用、按语义保留」历史叙述压成一行去向指引
- fix(monitoring): `trace` 落点锚 `--project-path`——从别处跑 `check --project-path B` 时 trace 由写进调用方 cwd 改为写进 B（#139，架构评审 2026-09-14「附带发现」第 1 条；违反 #95「收到的根要传到自己每个 IO/执行点」）——`DEFAULT_TRACE_FILE` 是相对路径而采集器全程按 cwd 解析（建目录/追加/读/stat），生产构造点又是 cwd 锚定的模块级单例 `getTraceCollector()`（`configureTraceCollector` 生产调用点为零），于是约束评估真跑了 B、trace 落进 A，且按 projectPath 直读 jsonl 正本的消费方（`status`、`constraints report`）读不到刚写的记录——#95 点名的假绿族，且表现为**静默写错位置**而非报错。守护面两头漏：闸 2 冻结的是「src 减 cli」的 cwd 站点而这里是相对路径**默认值**，闸 3 的正则只认名字以 `Path` 结尾的键 + 字符串字面量，`traceFile: DEFAULT_TRACE_FILE` 因「键名不带 Path、值是常量引用」不撞闸。**修法按票下 Agent Brief 裁决取 A（构造收根）**：`TraceCollectorConfig` 新增可选 `projectPath`，给了则缺省/相对的 `traceFile` 一律 `path.resolve(projectPath, …)`、没给保持 cwd 解析（跨仓消费者 studio 两处直调靠「API 进程 cwd 恰好是项目」成立，近期零破坏，远期在自己组合根传根迁移）；`DEFAULT_TRACE_FILE` 常量名不变、语义在 JSDoc 里明确为**项目相对片段**（不是可直接打开的路径）；四个组合根（CLI `check`、CLI `report`、`bootstrapHarness` 与 `bootstrapHarnessSync`）改为每调用一次锚根构造 `TraceCollector`，形状照仓内先例 `cli/commands/failure.ts` 的 `getRecorder(projectPath)`，harness 内部对单例的消费归零。`getTraceCollector()`/`configureTraceCollector()` 按裁决**保留为公共面**（JSDoc 写明 cwd 锚定的兼容语义），摘除留待 studio 迁移后的收缩票。**对外行为变化 1 处**（定级 **minor**，公共面为加性可选字段）：带 `--project-path` 从别的目录跑 `check`/`report` 时，trace 由落调用方 cwd 改为落目标项目 `.harness/logs/traces.log`；**不带 `--project-path` 的项目内使用逐字不变**（入口本就 `options.projectPath || process.cwd()`，锚定后解析出的仍是同一绝对路径，studio 缺省走这条）。存量杂散 trace 于 2026-09-15 判为垃圾数据并本机清理，无迁移。测试：新增 `src/monitoring/__tests__/trace-file-anchoring.test.ts`（真 fs 不 mock IO，两态各钉：收根→锚项目、不收根→锚 cwd，另覆显式绝对/相对 `traceFile` 与「读侧同锚」）+ `project-path-anchoring.test.ts` 站点 3（**红基线实测**：cwd=A 跑 `check --project-path B`，B 侧新增记录数 0、A 侧长出 `A/.harness/logs/traces.log`；并钉读-写闭环——`status --project-path B` 的 `记录数` 等于 B 正本行数且列出 check 落的约束）；闸 3 两臂扩形（键名后缀 `Path` → `Path|File|Log` + 新增 `\w+(Path|File|Log)\s*[:=]\s*[A-Z][A-Z0-9_]+` 常量引用臂，只扫使用点不冻结常量定义）并带**反证用例**（未锚的 `traceFile: DEFAULT_TRACE_FILE`、`errorLog: 'logs/…'`、`summaryPath = './…'` 三形各顶闸一次；锚定形、绝对值、小写值、`maxFileSize` 不误报——实测全下游层命中恰好 3 处 = 既有 validator 豁免 + 本票病灶 + `trace-analyzer` 的 `summaryFile`，零假阳性）；新增闸 4（`src/cli`、`src/hooks` 生产代码对 `getTraceCollector()` 零消费）。`trace-analyzer` 的 `summaryFile` 同型病灶与 `DEFAULT_FAILURE_LOG_FILE` 一并登记豁免不修（harness 侧无生产写点，与 studio 侧三件套迁移同票，#139 Out of scope）。随迁：`check.test.ts` 不再重定向全局收集器（改断言项目正本、切掉夹具种子行）、`check-read-count.test.ts` 删除已失效的重定向（写入不经读取计数器，冻结读取表逐行不变）、`check-drift` / `check-skip-output` 的 traces 模块替身由 `getTraceCollector` 换为 `TraceCollector` 构造形。验收：`npx jest` 全量套件全绿、`tsc --noEmit` 与 `eslint src` 零错、`node bin/harness.js check` 通过。文档：`src/monitoring/CONTEXT.md` 新增落点锚根约定条（含四处可检面）、`src/cli/commands/CONTEXT.md` 的 #88 注入形状与 #95 三道闸→四道闸、`src/hooks/CONTEXT.md` 组合根形状、`src/gates/CONTEXT.md` 的 #95 适用域描述
- fix(security): `harness security` 缺省扫描命令断链修复——`?? ''` 把「未提供」兜成空串，令三级兜底恒停在第二级、自动探测不可达（#138）——`src/gates/security.ts` 构造器 `scanCommand: config.scanCommand ?? ''` 把「未给命令」表示成空串，而 `scan()` 的三级优先级用 `??`（空串非 nullish），于是链恒在第二级返回 `''`、第三级 `detectScanCommand()` 全仓无可达路径；`runScan('')` 拿空串去 `exec` 直接抛 `The argument 'file' cannot be empty. Received ''` → 门禁自身报错退出而非判定，`harness security` 不带 `--scan-command` 必失败。根因是类型层不诚实：`SecurityGateConfig.scanCommand` 本就是可选，但 `this.config`/`getConfig()` 声明为 `Required<SecurityGateConfig>`，逼构造器兜空串。**修法**（票面方案 A，非 `||` 短路——不把空串当有意义的缺省位）：构造器 `config.scanCommand || undefined` 把空串归一为 `undefined`，判定链保持 `??`（「未提供 = undefined」的唯一表示），`this.config`/`getConfig()` 类型随之由 `Required<SecurityGateConfig>` 改回 `SecurityGateConfig`（该类型仅 `scanCommand` 可选、其余三字段本已必填，故「scanCommand 可选、其余必填」正是类型本身，无需 `Required<>` 也无需新造别名）；`detectScanCommand()` 恢复可达、内容不改（仍常量返回 `npm audit --json`）。**对外行为变化**（缺省路径，定级 **fix/patch 非 breaking**）：不给 `--scan-command` 的 `harness security` / `harness security audit` 由「恒报错退 1」变为「按探测命令真跑一次扫描并按结果判定、退出码由扫描结果决定」；显式 `--scan-command <cmd>` 与 `context.securityScanCommand` 两条路径逐字不变；`--scan-command ""` 视为未提供 → 落到探测。定级核实：本仓 `.github/workflows/` 仅 `harness-check.yml` 调 `harness check --preset`，无任一处缺省 `harness security`；`harness security` 只出现在 README 示例与 `dist` 产物注释（非调用点）；`getGate()`/`listRegisteredGates()`/`runGates` 生产零调用点，security 门禁只经其 CLI 命令进入，`harness check` 不受影响——与票下 Agent Brief「harness 与 studio 两仓 CI、studio-config 部署脚本均核实无缺省 security 调用点」一致，故无流水线依赖「security 必红」语义。测试：新增 `src/gates/__tests__/security-default-scan.test.ts`（门禁级）与 `src/cli/commands/__tests__/security-default-scan.test.ts`（命令级），**先红后绿、与修复同 commit**。替身不落 `child_process`（`security.test.ts` 顶层整体 mock 后 `exec('')` 照样回调桩数据、永不抛错——正是缺省路径一直亮绿灯的机制），改在 exec 唯一出口 `src/utils/exec` 的 `execAsync` 上打缝：记录门禁「实际下发去执行的命令字符串」并断言缺省时恰为 `detectScanCommand()` 的返回值 `npm audit --json`（断链时是 `''` → 红），缝替身对空串复现真实 exec 抛错以保持红/绿分界诚实；覆盖缺省 scan/evaluate、context 优先级、config 优先级、`--scan-command ''` 归一、`getConfig().scanCommand` 为 `undefined`、CLI `security`/`security audit`/显式命令四形。既有 `security.test.ts` 两条（context 命令 / config 命令）逐字保留。公共类型面：`SecurityGateConfig` 名字清单零改动（`public-type-surface` 闸绿），仅 `getConfig()` 返回类型 `scanCommand` 由 `string` 收窄为可选——本仓零消费者的纯类型面。验收：`npx jest` 全量 173 套件全绿（新增 12 项，含 `layering.test.ts` 逐目录分层闸——CLI 级用例落 cli 层避免 gates→cli 越界）、`tsc --noEmit` 与 `eslint src` 零错。文档：`src/gates/CONTEXT.md` 注意事项补 `SecurityGate` 三级优先级与实际生效语义一条（防后续读码者再以为探测是死代码）。范围外按票面另开票：按包管理器探测（读 lock 选 pnpm/yarn audit，`detectScanCommand` 维持常量）、`auditDetails` 的吞错（扫描失败仍 exit 0）、severity 阈值语义与漏洞 JSON 解析格式

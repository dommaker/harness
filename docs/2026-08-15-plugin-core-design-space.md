...
- **收益**：新增门禁/守卫 = 只加一个定义（定义即注册），不再手改命令入口三处；顺序/开关可声明；checker-as-guard 接入点统一（studio#82 D4 的 checker 接线从「在 `runCompletionGuards` 函数体加 if」变「注册一个守卫」）。
...
- **与 `simplest_solution_first` 冲突**：若门禁长期只有 6 个且不新增，统一化是为「一次性抽象」的 speculative 重构。缓解判据：`checker-as-guard`（studio#82 D4）是否确定要接入——若确定，统一化就顺带消灭一个硬编码点，价值成立；否则可缓。
...
- **收益**：多段变更原子回滚 + 提案级可审计回滚；studio#82 D6「进化提案落地通道」的天然挂点（一个提案改多段时，回滚要原子地一起回退）。
...
- **超前建模**：当前变更通道主要是 retire（单段、删段即恢复），提案落地通道（studio#82 D6）尚未实现；若先建 inverse 框架而无消费方，是 speculative。缓解判据：挂点应落在 studio#82 D6 实现时（邻接立项），不单独先建。
...
- **定位**：只摘「可逆 effect 建模」单点，挂到 studio#82 D6 提案落地通道，其余不动。
...
- **相对收益/代价**：收益面最广（hook/gate/命令三类故障类一次性消灭，checker-as-guard 接入点统一）；代价中低，且为 studio#82 D4 提供现成接线点。
...
|---|---|---|---|
| 一句话定位 | 单点 inverse 回滚，挂 studio#82 D6 | 定义即注册 + 构建期闭环，消灭手工列表/入口 | 声明式插件 + 依赖解析 + 静态装配 |
...
| 是否引依赖 | 否 | 否 | 否（自研） |
| 与 studio#82 关系 | 直接挂 D6 提案落地通道 | 为 D4 checker-as-guard 接线铺路 | D4 + 未来依赖拓扑预留 |
...
1. **守卫统一化的前置条件**：`checker-as-guard`（studio#82 D4）是否确定接入？确定 → 选项 1 价值成立；不确定 → 选项 1 是 speculative，可缓。
2. **闭环推广是否上升为 ADR**：若选 B，是否把「注册型能力一律『定义即注册 + 构建期闭环』」固化为跨模块约定（可选 2c）。
3. **inverse 建模的挂点**：选项 3 是否必须等 studio#82 D6 提案落地通道实现后再落地（避免超前建模），还是可先建「提案记录 inverse 字段」的 schema 预留。

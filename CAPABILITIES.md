...
- check (7)：必须带真实 checker，注册表闭环。Iron Laws (3) 违规阻断；Guidelines (4) 违规告警。（harness#174/ADR-0028：10 → 7，删 incremental_progress/no_implementation_without_requirement/no_bypass_checkpoint 及其 checker）
- prompt (9)：纯文本行为约束，带角色路由与适用性标签，不占检查位、不产生 trace 统计。（harness#174/ADR-0028：16 → 9；checker 条目的 promptInjection 冗余文本同步清除，仅 no_completion_without_verification 保留注入文本）

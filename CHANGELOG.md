...
### Changes
> **发布级别裁决（2026-09-09）**：本车含四处 breaking（ADR-0017/0018/0019/0022 的公共面删除 + 评审 A1/A2 追加的死旗帜与死类型），原各条目与 ADR 的「按 major 发布 / 影响版本：下个 major（2.0.0）」判定被人类裁决推翻，改按 **1.6.0（minor）** 发布。依据：本包唯一消费者为 studio，采纳由 ship-chain 阶段 2 显式 bump 完成，`monitor-system-probes` 调用点随 studio#459（`32e34b39`）同批部署翻转。代价记档：breaking 内容落在 `^1.x` 可解析范围内，任何按范围解析的新增安装都会拿到它。

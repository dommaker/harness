...
5. **剩余边界（已收口：#131，2026-09-15）**：子路径入口的**值面**无对等冻结闸——`public-exports.test.ts` 的运行时清单只钉 `Object.keys(require('../index'))` 即包根，`./gates` 的 5 个内联 `create*Gate` 与 `getEffectiveGates` 等只被本闸的写法白名单看见、不被清单钉住。同类定级断言（「这个函数不在公开面上」）在子路径值面仍无闸可跑。收口前，值面删除的定级同样须逐条附 `exports` 各入口源文件 + 已发布产物的 `.d.ts` 双证据。
   - **收口**：新增 `src/__tests__/public-value-surface.test.ts`，四个子路径入口（`./core` / `./presets` / `./context` / `./gates`）的运行时导出键集合逐入口冻结，入口清单与类型面闸同口径——从 `package.json` 的 `exports` 派生，新增子路径未登记即红。包根仍归 `public-exports.test.ts`（同一入口两份清单必然漂移，且那条判定要逐字保留）；「两边都不钉」由登记闸 + 委派闸钉住。清单不手抄：`src` 侧 ts-jest 运行时键与 tsc 产物（各 `exports[*].require` 指向的 `.js`）逐入口对撞，106/29/4/4/24 全部逐字一致。人工双证据口径（上条末句）随之退役。
   - **落闸前实测校正本条前提**：本条举的例子（删 `./gates` 的一个 `create*Gate` 当前无测试会红）不成立——`src/index.ts` 经 `'./gates'` barrel 再导出这些符号，删它会让包根那条闸红，只是红名报 `.`、不报 `./gates`。真实的零覆盖面是包根未转发的符号：`./core` 的 `ConstraintChecker` / `ProjectConfigLoader` / `constraintChecker` / `getCapabilitiesMode` / `getGovernanceConfig` / `loadRawProjectConfig` / `resolveContextFiles` 七个，`./presets` 的全部四个（`STRICT_PRESET` / `STANDARD_PRESET` / `RELAXED_PRESET` / `PRESETS_BY_NAME`）——实测删 `STRICT_PRESET` 时既有三道公共面闸全绿。反方向四个子路径都漏：向 `./gates` 新增一个运行时导出，既有闸也全绿。`getEffectiveGates` 在本票开工前已随门禁声明式配置面的收缩删除，不在冻结清单内。
   - 反向验闸四例各红一次、diff 指名到入口与符号：删 `./gates` 的 `createReviewGate`、向 `./gates` 注入 `createScratchGate`、删 `./presets` 的 `STRICT_PRESET`、`package.json` 新增 `./monitoring` 子路径（落登记闸）。每次注入后按 sha1 还原现场。另测类型面改动不连带抹掉本闸：向 `src/index.ts` 注一条 `export type { DynamicTask }` 令 `public-exports.test.ts` 套件 TS2578「failed to run」，本闸在同一注入下 7 项照跑。
   - 非 breaking：纯测试 + 文档记账，零生产代码改动、零公共面变化。

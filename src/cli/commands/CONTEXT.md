...
- **scaffold 三态模型（harness#132 grilling 落定，待实施）**：init/validate 的「目标在不在场→写或告知」判定（8 站点 = init 6 + validate 2）收口为 scaffold 模块。术语：**managed file**（受管文件）= harness 模板拥有正本、用户可持有的落盘文件；三态 = `created`（不在场→落盘）/ `exists`（在场→告知跳过，**不细分内容是否等于模板**，内容比较是独立特性）/ `manual`（在场→打印片段请用户手工合并）。纪律：scaffold 不持有覆盖/跳过策略（skip 旗帜由命令层翻成 plan 列表，scaffold 只见 plan）；模板留代码内收模板数据模块，不落 `templates/`（其无运行时消费者、integrity 清单不覆盖）；GH Actions 站点冲突分支改为打印完整 workflow 全文（修打印≠落盘的同源漂移，#103 判据）；CLAUDE.md/AGENTS.md 标记化幂等写（读-改-写形状）不纳入 scaffold；死选项 `-t/--type` 随之删除。GitLab CI 接线与 pre-push hook 为已知缺口，另开票
- **knowledge view 收口模型（harness#133 grilling 落定，待实施）**：knowledge 11 个子操作的「取数 + json 投影 + 人读排版 + 退出码」四件事拆分——子操作只产 `{ data, human() }`；术语 **display model** = `human()` 返回的结构化人读视图（`{ sections: [{ title, rows }] }` 形状，非预排版字符串），chalk 上色与「键→中文 label/颜色」映射（维度 label、成熟度颜色、规则 label）全部收进单一 view 模块，路径解析与 store 构造同收一处。json schema **现状冻结**：11 个顶层形状逐字不动（双仓核实 `knowledge --json` 零程序化消费者，studio 走 library 直调；统一包壳是纯 churn，删除 `--json` 死面按 ADR-0022 口径另票评估）；字段清单显式冻结、增删须改清单；两投影一致性单点断言——display model 出现的字段名必须在 json 面存在或登记豁免。与 #134（KnowledgeAudit 收 store）独立落地，唯一交点 `knowledgeAudit` 命令函数，后到者适配
